...
class Plugin(object):
    ...
    def __init__(self, pm):
        ...
        for line in self.blk_file:
            blacklist.add(line.strip()) #defines a blacklist of terms



    @staticmethod
    def register_events():
        return [Events.Command("image", desc="Search Gelbooru! Refer to the Gelbooru Cheat Sheet for tags http://gelbooru.com/index.php?page=help&topic=cheatsheet"),
                Events.Command("nsfw", desc="spicier search"),
                Events.Command("remove", Ranks.Mod,
    ...
    async def search(self, message_object, text, nsfw):
        ...
        if len(response.json()) is 0:
            await self.pm.client.send_message(message_object.channel, "Can't find image")
            return
